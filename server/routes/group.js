...
router.post('/requests/:id/invite', requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.log(`\n🎯 Sending invitation from ${inviter.sid} to ${groupRequest.sid}`);
    ...
    try {
      ...
      // Auto-create chat group when invitation is sent
      const existingGroup = await db.collection('chat_groups').findOne({
        members: { $all: [inviter.sid, groupRequest.sid] }
      });

      if (!existingGroup) {
        await db.collection('chat_groups').insertOne({
          name: `${groupRequest.major} Study Group`,
          courseCode: groupRequest.major,
          members: [inviter.sid, groupRequest.sid],
          createdBy: inviter.sid,
          createdAt: new Date(),
                                                     lastMessageAt: new Date()
        });
        console.log(`✅ Chat group created for ${inviter.sid} and ${groupRequest.sid}`);
      }
